  val spark = SparkSession.builder.appName("proj - module").getOrCreate()  
    ...
    || (count(transfer_key) > 20) \
    || (sum(transaction_amt) >250).withColumn("Risk_level",lit("HR"))
    ...
    val MedDF = finalDf.filter("(sum(transaction_amt) > 600 & sum(transaction_amt) < 1000 && transaction_type = 'INN') || (sum(transaction_amt) > 500 && sum(transaction_amt)<800 && transaction_type = 'OUT') ")\
      || (count(transfer_key) > 10) &&  count(transfer_key) < 20 ) || (sum(transaction_amt) > 100 && sum(transaction_amt) <250)) \
    ...
    || count(transfer_key) < 10).withColumn("Risk_level",lit("LR"))
